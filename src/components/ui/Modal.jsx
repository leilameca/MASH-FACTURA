...
export function Modal({ open, title, children, footer, onClose, size = 'md' }) {
  ...
  return (
    <div className="modal-overlay fixed inset-0 z-[120] flex items-end justify-center bg-black/50 px-0 backdrop-blur-sm md:items-center md:p-8">
      <section
        className={cn(
          'modal-sheet modal-enter grid w-full overflow-hidden rounded-t-3xl bg-white shadow-mashXl [grid-template-rows:auto_1fr_auto] md:rounded-2xl',
          size === 'sm' && 'md:max-w-md',
          size === 'md' && 'md:max-w-xl',
      ...
      >
        <header className="flex items-center justify-between gap-4 border-b border-mash-border bg-white px-4 py-4 sm:px-6 sm:py-5">
        ...
        <div className="modal-content min-h-0 overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
          {children}
        </div>
        {footer ? (
          <footer className="modal-footer border-t border-mash-border bg-white px-4 py-3 sm:px-6 sm:py-4">
